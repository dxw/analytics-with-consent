...
class Scripts implements \Dxw\Iguana\Registerable
{
	public function register(): void
	{
		...
		add_action('wp_head', [$this, 'addGA4'], 10, 0);
		add_action('wp_head', [$this, 'addGTM'], 10, 0);
	...
	public function addGA4(): void
	{
		...
		if ($apiKey && $productType && $ga4Id) {
			wp_print_script_tag([
				'async' => true,
				'id' => 'awc_gtag',
				'src' => esc_url('https://www.googletagmanager.com/gtag/js?id=' . $ga4Id),
			]);
		}
	}
	...
	public function addGTM(): void
	{
		...
		if ($apiKey && $productType && $gtmId) {
			...
			wp_print_inline_script_tag($gtmTrackingScript);
		}
	}
